...
_final: prev: {
  pythonPackagesExtensions = (prev.pythonPackagesExtensions or [ ]) ++ [
    (_pyFinal: pyPrev: {
      # The selected vLLM snapshot requires huggingface-hub >=1.31.0 for
      # its httpx re-export. The pinned nixpkgs substrate is held stable for
      # the XPU runtime and only carries 1.26.0, so update this dependency.
